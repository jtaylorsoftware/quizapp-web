...
import { MultipleChoiceAnswer, MultipleChoiceQuestion, MultipleChoiceResult, Question, ResultAnswer } from 'api'
...
describe('ScoredQuestion', () => {
  ...
  it('renders the question text', () => {
    render(<ScoredQuestion index={0} question={mockQuestion} result={mockResult} />)
    expect(screen.queryByText(`1. ${mockQuestion.text}`)).not.toBeNull()
  })
  it('renders the correct answer when given', () => {
    render(<ScoredQuestion index={0} question={mockQuestion} result={mockResult} />)
    expect(
      screen.getByText(`Correct answer: ${(mockResult as MultipleChoiceResult).correctAnswer! + 1}`),
    ).not.toBeNull()
  })
  it('does not render the correct answer text if it is not given', () => {
    render(<ScoredQuestion index={0} question={mockQuestion} result={{
      ...mockResult,
  ...
  it('renders whatever answers are given', () => {
    render(<ScoredQuestion index={0} question={mockQuestion} result={mockResult} />)
    ;(mockQuestion as MultipleChoiceQuestion).answers.forEach((answer: MultipleChoiceAnswer) => {
